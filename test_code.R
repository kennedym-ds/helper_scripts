...
detect_anomalies <- function(data, window_size = 30, k_step = 3, k_roll = 2.5, time_window = 100, acf_lag_max = 50, value_col = "Value") {
    ...
    mean_shift_indices <- which(abs(value - data$Rolling_Mean) > (k_roll * data$Rolling_SD))
   ...
   # 4. Change Point Detection (Parametric)
    ...
    # Classify based on change points, using the correct Time index
    ...
    if (length(significant_lags) > 0) {
        data$Classification[length(significant_lags)] <- "Oscillation"
    }
...
main <- function() {
    ...
    synthetic_data_ns_l <- generate_synthetic_soch_data(
        ...
        outlier_points = c(50, 150),
        sd_shift_points = list(list(start=100, end=150, multiplier=2)) #Added to ensure errors
    )
    ...
    preprocessed_data_ns_l$Differenced_Seasonal_Value <- c(rep(NA, 20), diff(preprocessed_data_ns_l$Detrended_Value, lag = 20))

    #Check Stationarity on TRANSFROMED DATA
    if (!is_stationary(preprocessed_data_ns_l, value_col = "Differenced_Seasonal_Value")) {
        ...
        print("Data is stationary after transformation")
    }

    # --- Scenario 3: Stationary Data without Limits ---
    cat("\n--- Scenario 3: Stationary Data without Limits ---\n")
    synthetic_data_s_nl <- generate_synthetic_soch_data(
        n = 200, base_mean = 50, base_sd = 4, ucl = NULL, lcl = NULL,
        shift_points = list(list(start = 100, end = 120, mean = 58))
    )
    ...
    synthetic_data_ns_nl <- generate_synthetic_soch_data(
        ...
        lcl = NULL,
        stationary = FALSE,
        trend_params = list(slope = -0.15, start = 50, end = 150)
    )
    ...
    if (!is_stationary(preprocessed_data_ns_nl, value_col = "Detrended_Value")) {
        analyzed_data_ns_nl <- detect_anomalies(preprocessed_data_ns_nl, value_col = "Detrended_Value")
        plot_soch_with_anomalies(analyzed_data_ns_nl, value_col = "Detrended_Value", title = "Scenario 4: Non-Stationary without Limits")
