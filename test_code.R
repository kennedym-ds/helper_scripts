...
generate_synthetic_soch_data <- function(n = 500,
                                         ...
                                         lcl = 85,
                                         seed = 42,
                                         shift_points = NULL, #List of lists, each sublist has start, end and new mean
                                         trend_points = NULL, #List of lists, each sublist has start, end and, slope
                                         sd_shift_points = NULL, #List of lists, each sublist contains start, end and multiplier
                                         oscillation_points = NULL, #List of lists, each sublist contains start, end, period, amplitude
                                         outlier_points = NULL, #A vector of time indices for single point outliers
                                         missing_points = NULL #A vector of time indices to make NA
) {
    set.seed(seed)
...
detect_anomalies <- function(data, window_size = 30, k = 3, time_window = 100, acf_lag_max = 50, value_col = "Value") {
    ...
    np_changes <- cpt.np(value, method = "PELT", penalty = "SIC") #Using SIC here
    ...
    if(length(cpts(np_changes)) > 0) {
        data$changepoint_np <- any( (max(data$Time) - data$Time[cpts(np_changes)]) <= time_window)
    }
    ...
    data$Anomaly <- data$Out_of_Control |
        ...
        !is.na(data$changepoint_np) & data$changepoint_np
...
plot_soch_with_anomalies <- function(data, value_col, title = "Soch Chart with Anomaly Detection") {
    p <- ggplot(data, aes(x = Time, y = .data[[value_col]])) + #Use the passed value_col
        geom_line() +
        geom_point(aes(color = Anomaly)) +
        geom_ribbon(aes(ymin = LCL, ymax = UCL), fill = "grey", alpha = 0.3) +
        geom_hline(aes(yintercept = CenterLine), linetype = "dashed") +
        # You can add vlines for changepoints if you store them, e.g.,
        # geom_vline(xintercept = data$Time[data$changepoint_mean], color="blue", linetype="dotted") +
        labs(title = title, x = "Time", y = value_col, color = "Anomaly Detected") +
        theme_bw()

    #Optional to show the rolling stats
    # p <- p + geom_line(aes(y=Rolling_Mean), color = "blue")
    # p <- p + geom_line(aes(y=Rolling_SD), color = "green")

    print(p)
    return(p) # Return the plot object for further manipulation, if needed.
}
...
main <- function() {
    ...
    if (is_stationary(preprocessed_data)) {
        ...
        print(paste("Number of anomolies detected", sum(analyzed_data$Anomaly)))
